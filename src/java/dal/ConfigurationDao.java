...
public class ConfigurationDao extends DBContext {
    ...
    public void saveGlobalConstants() {
    Connection con = null;
    PreparedStatement stmt = null;
    ResultSet rs = null;
    try {
        ...
        // Check if any data exists in the Configuration table
        String checkSql = "SELECT COUNT(*) FROM Configuration";
        ...
        if (count == 0) {
            // Insert default values into the Configuration table
            String insertSql = "INSERT INTO Configuration ([key], [value], createdAt, updatedAt) VALUES (?, ?, ?, ?) ";

            Object[][] constants = {
                {"GLOBAL_GENERATE_KEY", "ODB_Group1"},
                {"GOOGLE_RECAPTCHA_SITE_KEY", "6LfmnoEkAAAAABOUgIhNz6MpA-xs2ABmGyRc3225"},
                {"GOOGLE_RECAPTCHA_SECRET_KEY", "6LfmnoEkAAAAAKOsjUoLCveSkAk4CHgLeWMxLEIl"},
                {"SMTP_HOST", "smtp.gmail.com"},
                {"SMTP_PORT", "587"},
                {"SMTP_ACCOUNT_EMAIL", "nva.shinki@gmail.com"},
                {"SMTP_ACCOUNT_PASSWORD", "bomwkvhvyixkazrn"},
                {"DOMAIN", "localhost"},
                {"PORT", "9999"},
                {"HOST", "localhost:9999/ODB_Online"}
               
            };

            for (Object[] constant : constants) {
                stmt = con.prepareStatement(insertSql);
                stmt.setString(1, (String) constant[0]);
                stmt.setString(2, (String) constant[1]);
    ...
    public static void main(String[] args) {
        new ConfigurationDao().saveGlobalConstants();
